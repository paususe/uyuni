...
public class RhelUtilsTest extends JMockBaseTestCaseWithUser {
    ...
    private static String ALINUX_RELEASE =
    ...
    public void testParseReleaseFileAlinux() {
        Optional<RhelUtils.ReleaseFile> os = RhelUtils.parseReleaseFile(ALINUX_RELEASE);
        assertTrue(os.isPresent());
        assertEquals("Alinux", os.get().getName());
    ...
    private void doTestDetectRhelProduct(String json, SetupMinionConsumer setupMinion, Consumer<Optional<RhelUtils.RhelProduct>> response)
        ...
        String centosReleaseContent = map.get("cmd_|-centosrelease_|-cat /etc/centos-release_|-run")
                ...
                .getStdout();
        String alinuxReleaseContent = map.get("cmd_|-alinuxrelease_|-cat /etc/alinux-release_|-run")
                .getChanges(CmdResult.class)
                .getStdout();
        ...
        Optional<RhelUtils.RhelProduct> prod = RhelUtils.detectRhelProduct(minionServer,
                ...
                Optional.ofNullable(alinuxReleaseContent));
    ...
    public void testDetectRhelProductAlinux() throws Exception {
        doTestDetectRhelProduct("dummy_packages_redhatprodinfo_oracle.json",
                null,
                prod -> {
                    assertFalse(prod.get().getSuseProduct().isPresent());
                    assertEquals("Alinux", prod.get().getName());
